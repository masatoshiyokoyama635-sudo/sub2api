export default {
    accounts: {
      ...
      usageWindowsHint: "“5h / 7d”是上游账号（如 OpenAI ChatGPT、Claude）官方的滚动用量窗口限制，由上游对账号设定，并非 sub2api 配置，也与你映射的模型无关。窗口滚动到期后用量会自动重置。下方 Turn-State 条目按模型显示候选；百分比与倒计时表示剩余有效期，不是命中率或模型质量。紫色表示匹配候选形态，琥珀色表示未匹配；形态不能确认实际响应模型。",
      ollamaCloud: {
      ...
      status: {
        ...
        turnStateHold: '缺候选暂停 · {model} 在本账号上暂停，猎手正补充候选；取得候选即恢复，到期后仍缺候选会再次暂停',
        turnStateHoldShort: '缺候选'
      },
      tempUnschedulable: {
        title: '临时不可调度',
      ...
      openai: {
        ...
        turnStateAutoDesc:
          "开启后，系统按账号和模型维护候选：当前形态规则接受个人账号的 292 字符 / 10 块与 Team 账号的 332 字符 / 12 块。检测到非候选形态时，尝试注入最近的有效候选；注入后仍返回非候选形态则按规则失效并尝试下一条，全部候选失效时停用账号并记录原因。候选自铸造起默认 1 小时有效，过期后等待新候选；开启后忽略手填值。仅覆盖 HTTP 路径。匹配或未匹配这些形态都不能确认实际响应模型，也不保证模型质量。",
        turnStateAutoTakeover: '已由自动接管',
        turnStateModelsEmpty: '(拉不到模型列表)',
        turnStateOverrideConfigured: '已配票的模型：{models}',
        turnStateHunter: "Turn-State 猎手",
        turnStateHunterDesc:
          "票到期前开窗，经所选代理新建会话探测，取得匹配候选形态的状态即入池：当前规则接受 292 / 10 块与 Team 的 332 / 12 块。响应头到手即断，主要成本是每次探测的输入 token（含模型 base prompt）。猎手开启时，有候选就对该模型的会话尝试注入；每小时有上限，无近期真实请求的模型不猎。探测新建代理连接，轮换端点据此换出口；固定出口每轮探一次，返回非候选形态的出口按规则冷却 7 天。候选形态不保证实际响应模型，非候选形态也不是模型变化的确认。",
        ...
        turnStateHunterRotating: "勾选每次新连接换出口的代理（webshare 的 -rotate 自动识别）；未勾选按固定出口处理，每轮探一次，返回非候选形态的出口冷却 7 天。此标记只影响猎手探测，正常请求仍走账号配置的代理。",
        ...
        turnStateHunterHold: "缺少可用候选时暂停模型调度",
        turnStateHunterHoldDesc:
          "要猎的模型没有可注入的有效候选时，把该模型在本账号上暂停一个空闲窗口（idle_minutes）并让请求换号（没有其他账号则返回 503）；到期后新请求仍缺候选就再次暂停，猎到候选立即恢复。其他模型不受影响；这属于候选可用性控制，不代表已确认模型变化。",
        turnStateRecovery: "账号出口状态恢复探测",
        turnStateRecoveryDesc:
          "使用账号原代理按随机间隔探测；连续若干次返回匹配候选形态的状态（当前为 292 / 10 块、Team 332 / 12 块）后标记形态恢复，连续失败达到阈值则冷却。独立于猎手开关，只记录标记与日志，不修改配置；标记后停止，真实流量再次返回非候选形态时重新计数。该标记不保证实际响应模型或模型质量。",
        turnStateRecoveryUsageKey: '记账 API Key ID（留空沿用猎手配置）',
        turnStateRecoveryUsageKeyDesc: '留空时使用猎手配置中的记账 Key，猎手关闭时也会沿用；两处都未配置才不记录本地探测用量。使用的 Key 按标准计费与额度规则处理。',
        ...
        turnStatePool: {
          ...
          recoveryDone: "状态形态已匹配 · {time}",
          hunterGateIdle: '无流量·暂停',
          hunterGateHeld: "模型暂停·补充候选中",
