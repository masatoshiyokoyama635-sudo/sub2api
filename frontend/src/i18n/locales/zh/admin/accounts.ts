export default {
    accounts: {
      turnStateHunter: {
        candidatesTitle: "后台猎手候选",
        sharedCache: "猎手候选保存在共享缓存中，服务重启后或其他实例仍可读取；有效期以候选到期时间为准。",
        localCache: "猎手候选当前保存在本进程缓存，服务重启后清空。",
        recoveryUsageKey: "记账 API Key ID（留空沿用猎手配置）",
        configure: "配置猎手",
        shortcutHint: "独立后台运行，可使用 IP 管理中的轮换代理。",
        attemptResponseModel: "上游返回模型",
        authBlocked: "凭据验证失败，已暂停探测；更新凭据后再试。",
        rateLimitUntil: "上游限流，暂停至",
        "title": "后台猎手调度器",
        "description": "保存并开启后，后台按模型和代理持续补充候选，不需要用户请求触发。",
        "needsReuse": "后台猎手需启用 v2「采集并尝试 HTTP 复用」；恢复探测也可在「仅采集观测」模式下使用。",
        "models": "要猎的模型",
        "modelsPlaceholder": "每行一个上游模型名，最多 8 个",
        "autoModels": "按最近真实请求自动选择模型；启用后忽略上面的手动列表。重启后需要先有真实请求。",
        "proxies": "探测用代理",
        "proxiesHint": "从 IP 管理选择，可多选。仅影响猎手探测，正常请求继续使用账号绑定的代理。",
        "noProxies": "请先在 IP 管理添加代理，然后重新打开账号编辑。",
        "unavailableProxy": "未加载或已不可用的代理",
        "rotationHint": "勾选每次新连接更换出口的代理。探测会新建连接；实际换 IP 由代理服务决定。未勾选则按固定出口调度。",
        "autoRotation": "自动识别轮换",
        "hourlyLimit": "每小时探测上限",
        "gap": "探测间隔（秒）",
        "lead": "到期前开窗（分钟）",
        "idle": "等待真实流量",
        "retry": "一轮未命中后重试（分钟）",
        "usageKey": "记账 API Key ID（留空不记）",
        "effort": "探测思考强度",
        "defaultEffort": "默认（high）",
        "hold": "缺少可复用候选时暂停该模型调度",
        "holdHint": "仅暂停该账号上正在猎取且缺少有效候选的模型；请求会换号，无其他可用账号时返回 503。补到候选后恢复。",
        "lengthsHint": "沿用上面的候选长度列表；长度不代表模型质量。当前长度：",
        "recoveryTitle": "账号出口恢复探测",
        "recoveryHint": "独立于猎手开关，使用账号原代理低频探测。连续匹配候选规则后标记并停止；连续失败进入冷却，不会改写账号代理或自动修改配置。",
        "recoveryModel": "恢复探测模型",
        "recoveryModelAuto": "留空使用最近有真实流量的模型",
        "streakTarget": "连续符合条件次数",
        "cooldown": "连续失败冷却（小时）",
        "minInterval": "最短探测间隔（分钟）",
        "maxInterval": "最长探测间隔（分钟）",
        "lengthsRequired": "启用猎手或恢复探测需要至少一个有效候选长度。",
        "selectionRequired": "开启猎手需要选择探测代理，并填写模型或启用自动选择模型。",
        "selectionLimit": "最多选择 8 个模型、64 个探测代理。",
        "invalidNumber": "数值必须为输入框范围内的整数；留空使用默认值。",
        "invalidInterval": "恢复探测最长间隔不能小于最短间隔。",
        "invalidEffort": "请选择支持的思考强度。",
        "runtimeTitle": "后台猎手运行状态",
        "recoveryStatusTitle": "账号出口恢复探测状态",
        "enabled": "已开启",
        "disabled": "已关闭",
        "noRuntime": "已保存配置；后台尚未记录探测结果。",
        "nextAt": "下次调度",
        "hourCount": "本小时已探测",
        "gate": "当前调度状态",
        "updatedAt": "状态更新时间",
        ...
        "fresh": "候选仍在有效窗口",
        "capWait": "已达每小时上限",
        "runtimeError": "最近探测未成功；请检查代理可用性及服务日志。",
        "attemptTime": "探测时间",
        "attemptModel": "模型",
        "attemptProxy": "代理 ID",
        "attemptStatus": "HTTP 状态",
        "attemptLength": "状态长度",
        "attemptResult": "候选判定",
        "accepted": "符合当前候选规则",
        "notAccepted": "未通过候选规则",
        "attemptLatency": "耗时（毫秒）",
        "streak": "连续符合次数",
        "failStreak": "连续失败次数",
        "recoveredAt": "已标记时间",
        "coolingUntil": "冷却至",
        "attemptsHint": "只显示实际探测记录；符合候选规则不保证上游返回指定模型。",
        "unknownGate": "等待调度",
        "exitCount": "已记录固定出口数",
        "idleMinutes": "无真实请求多久后停探（分钟，-1 关闭）"
      },
      codexTurnStateStatus: {
        effectiveCollection: "已生效采集方式",
        collectionTitle: "主动采集结果",
        collectionActualOnly: "记录本模型实际发起的采集；因冷却或并发限制而未发出的请求不算一次采集。",
        collectionAttempts: "本模型采集次数",
        collectionResult: "最近采集结果",
        collectionStarted: "最近采集开始",
        collectionFinished: "最近采集结束",
        collectionNextEligible: "母凭据最早可再采集时间",
        collectionHTTPStatus: "采集 HTTP 状态码",
        collectionStateLength: "采集返回状态长度",
        collectionReasons: {
          in_progress: "正在采集",
          accepted: "已取得可用候选",
          missing_state: "未返回状态",
          length_not_allowed: "返回长度未被配置选中",
          invalid_format: "返回状态格式不符合候选条件",
          future_timestamp: "返回状态时间戳晚于当前时间",
          expired: "返回状态已超过本地保留期限",
          stale: "返回状态早于已有候选",
          model_mismatch: "响应声明模型与采集模型不同",
          model_unobserved: "未观测到响应模型",
          incomplete_response: "响应未完整结束",
          response_failed: "上游返回失败响应",
          body_too_large: "响应超过采集读取上限",
          transport_error: "采集连接失败",
          timeout: "采集超时",
          unauthorized: "上游拒绝认证（401）",
          forbidden: "上游拒绝访问（403）",
          rate_limited: "上游限流（429）",
          upstream_error: "上游请求失败",
          configuration_changed: "采集期间配置已变化，结果未采用",
          collection_failed: "采集失败",
          unknown: "未识别的采集结果",
        },
        effectiveMode: "已生效模式",
        effectiveIdentity: "已生效身份版本",
        effectiveLengths: "已生效候选长度",
        noSelectedLengths: "未选择（仅观测长度）",
        attemptsOnly: "复用次数记录候选选择尝试，不代表上游接受或保证返回指定模型。",
        totalReuseAttempts: "本模型累计复用尝试",
        unavailable: "暂无数据",
        lastSelection: "最近选择 / 跳过",
        lastRejection: "最近候选拒收",
        lastInvalidation: "最近候选失效",
        lastRequest: "最近完成请求对照",
        lastRequestOnly: "仅保留本模型最近完成的一次请求，不代表全部历史请求。",
        requestTime: "完成时间",
        requestId: "请求 ID",
        requestOutcome: "请求结果",
        requestFailed: "失败",
        requestCompleted: "完成",
        outboundState: "出站状态长度 / 来源",
        outboundModel: "出站模型",
        sourceNone: "无状态",
        sourceClient: "客户端状态",
        sourceCandidate: "缓存候选",
        selection: "候选选择结果",
        responseModel: "上游响应声明模型",
        responseModelUnknown: "未观测到响应模型",
        modelComparison: "与出站模型比较",
        modelMismatch: "响应声明与出站模型名不同",
        modelMatches: "响应声明与出站模型名一致",
        observationHistory: "历史长度观测",
        noObservations: "尚未观测到返回状态。",
        currentCandidate: "当前缓存候选",
        reuseNotEnabled: "已缓存候选；当前服务端配置未启用复用。",
        candidateLengthExcluded: "该候选长度未被当前配置选中，不参与复用。",
        candidateReuseAttempts: "当前候选复用尝试",
        reasons: {
          collection_cooldown: "该凭据采集被拒绝，冷却期间未发送正式请求",
          collection_rejected: "本次采集被上游拒绝，正式请求未发送",
          collection_pending: "该凭据采集尚未完成，正式请求未发送",
          reused: "已选择缓存候选",
          no_candidate: "无可用候选",
          expired: "已超过本地保留期限",
          length_not_allowed: "长度未被当前配置选中",
          observe_mode: "仅观测模式，未选择候选",
          client_state: "客户端已携带状态，跳过候选选择",
          client_continuation: "客户端续接请求，跳过候选选择",
          client_metadata_state: "客户端元数据已携带状态，跳过候选选择",
          accepted: "已接纳新候选",
          refreshed: "已更新候选",
          unchanged: "候选未变化",
          invalid_format: "状态格式不符合候选条件",
          future_timestamp: "状态时间戳晚于当前时间",
          stale: "早于已有候选",
          state_echo: "复用后的返回状态，不延长候选期限",
          upstream_rejected: "上游请求被拒绝，候选已失效",
          unknown: "未识别的原因",
        },
        title: "回合状态观测",
        refresh: "刷新观测",
        clear: "清空当前缓存",
        loading: "正在读取…",
        empty: "暂无正常请求观测；正常请求返回的状态会显示在这里，后台猎手候选与探测记录单独显示。",
        localOnly: "正常请求的被动候选与观测统计仅属于当前服务进程，重启后清空；猎手候选的存储范围另行显示。刷新不会请求上游。",
        loadFailed: "读取状态失败，请重试。",
        ...
        lastReused: "最近尝试复用",
        noCandidate: "无可复用候选",
        otherLengths: "其他长度 {count} 次",
        notReused: "尚未复用",
        savedSettings: "这里显示最近刷新读取到的服务端配置和观测结果；上方编辑内容需先保存。",
      },
      title: '账号管理',
      ...
      openai: {
        ...
        codexIdentityVersion: 'Codex 身份版本',
        codexIdentityVersionDesc: '默认使用 v1，保持原有身份算法。v2 保留线程、父子线程与窗口关系；切换后请新建会话。切回 v1 可回退，原有指纹种子会保留。',
        codexIdentityV1: 'v1（兼容，默认）',
        codexIdentityV2: 'v2（试验）',
        codexIdentityInherited: '继承凭证账号',
        codexIdentityBulkInherited: '影子账号继承凭证账号。批量修改版本时，请仅选择凭证账号；含影子的提交会被拒绝。',
        codexTurnStateCollection: "采集方式",
        codexTurnStateCollectionPassive: "随正常流量采集（默认）",
        codexTurnStateCollectionActive: "无候选时主动采集",
        codexTurnStateCollectionDesc: "仅复用模式可选。无候选且正式请求没有客户端状态或续接时，额外发送一次短采集请求；没有流量时不后台采集。",
        codexTurnStateCollectionLimits: "每次采集结束后，同母凭据至少间隔 180 秒；单次最多 20 秒，全进程最多 2 个并发。沿用当前账号与代理，不保证得到 332 状态或 Astra 响应。",
        codexTurnStateMode: "回合状态（v2）",
        codexTurnStateModeDesc: "默认从正常请求返回的状态中采集。复用仅用于 HTTP 请求未带状态时，并按凭据和模型隔离；主动采集需单独开启。",
        codexTurnStateOff: "关闭（默认）",
        codexTurnStateObserve: "仅观测与采集",
        codexTurnStateReuse: "采集并尝试 HTTP 复用",
        codexTurnStateLengths: "可复用候选长度",
        codexTurnStateLengthsDesc: "默认包含 292、332，可按 Team 实测调整。长度只用于筛选候选，不代表模型质量。逗号分隔，最多 8 个 100–2048 的整数；留空只观测、不选候选。",
        codexTurnStateLengthsInvalid: "请填写最多 8 个不同的整数（100–2048），用逗号分隔；也可留空。",
        codexTurnStateBulkDesc: "仅支持 v2 凭证账号，可同时将身份版本设为 v2。影子账号和 agentIdentity 账号不支持直接修改，请勿选入。未勾选时保留原配置。",
        codexTurnStateRequiresV2: "观测与复用需要 Codex 身份 v2，请同时选择 v2 或取消修改身份版本。",
