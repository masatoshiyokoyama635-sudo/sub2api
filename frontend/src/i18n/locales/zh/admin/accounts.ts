export default {
    accounts: {
      ...
      testPrompt: '提示词："hi"',
      pelicanTest: {
        runForHours: '运行时长（小时，1–168）',
        stopsAt: '自动停止时间',

        schedule: '定时测试',
        scheduleHint: '由服务端执行，关闭网页仍会运行。暂停后不再启动新一轮，已开始的测试会继续完成。',
        scheduleEnabled: '定时测试已开启',
        schedulePaused: '定时测试已暂停',
        everyMinutes: '每 {count} 分钟',
        pause: '暂停',
        resume: '恢复',
        nextRun: '下次执行',
        lastSuccess: '最近一次成功（保留记录内）',
        intervalMinutes: '间隔（分钟）',
        createSchedule: '创建定时测试',
        scheduleSaveHint: '保存上方的题目、模型、思考强度和并行数；间隔支持 1–10080 分钟。保留最近 50 份结果，超过 7 天自动清理，暂停后仍会清理。默认运行 24 小时后自动停止，间隔必须小于运行时长。保存或恢复后重新计算停止时间，等待一个间隔执行，调度约有 1 分钟延迟，不补跑错过的轮次。',
        scheduledHistory: '定时测试记录',
        preview: '查看 HTML / 动画',
        scheduleError: '定时测试操作失败',
