export default {
    accounts: {
      ...
      openai: {
        ...
        excelBPSMihomoDesc: '默认关闭。需先配置并启动 Mihomo 代理池，有会话标识时保持固定出口；无标识时为本次请求分配独立临时绑定，请求结束即释放。新会话按近期成功率、连通率和负载选节点，健康会话保持原出口；首次断流冷却 5 分钟，短时间重复断流逐步延长至 30 分钟；异常节点经过探测和冷却后再使用，受影响会话在进行中的请求结束后自动换出口。仅确认请求尚未发送时额外重试一次，不回退直连。空闲 30 分钟释放绑定，重启后重新分配。动态 IP 供应商须支持粘性出口。关闭后恢复账号原代理。',
        excelBPSAutoDisableOn403: '遇到 BPS 403 错误时自动关闭协议',
        excelBPSAutoMoveOn403: '遇到 BPS 403 错误时自动调整分组',
        excelBPSAutoMoveOn403Desc: '默认关闭. 触发条件与自动关闭协议相同, 两项可独立或同时勾选. 移入目标分组时会退出其他所有分组, 也可选择退出全部分组. 不禁用账号, 不重试当前请求. 目标分组失效或配置已变更时不调整分组.',
        ...
        codexTurnTicket: 'Codex 780 门票',
        codexTurnTicketDesc: '显示已配置模型的门票状态。仅在启用缺票拦截时，没有有效门票的模型才会暂停调度。',
        ticketProbeNext: '下次允许探测：',
        ticketStandbyExpires: '备用到期：',
        ticketProbe: { success: '采集成功', token_error: '账号令牌不可用', response_incomplete_or_error: '上游响应失败或未完整结束', invalid_state: '票据形态或有效期不符合要求' },
