export default {
    ...
    usage: {
      ...
      turnStateCopied: 'Turn-State 已复制',
      turnStateOverridden: '本次出站带的是账号级覆写值',
      turnStateOverriddenShort: '覆写',
      turnStateSent: 'Turn-State 出站',
      turnStateHint: "{chars} 字符（密文 {blocks} 块）；PKCS#7 下明文落在 {min}–{max} 字节。当前候选形态为 292 字符 / 10 块与 Team 的 332 字符 / 12 块。这里只提示状态形态，匹配或未匹配都不能确认实际响应模型，也不保证模型质量。铸于 {minted}",
	  ...
	  turnStateAll: '全部 Turn-State 状态',
	  turnStateMintedOnly: '上游铸出新值',
	  turnStateHealthyOnly: "新值长度匹配候选（292/332）",
	  turnStateSuspectOnly: "新值长度未匹配候选（非 292/332）",
	  turnStateSentOnly: '出站带了 Turn-State',
	  turnStateInjectedOnly: '出站为注入覆写',
