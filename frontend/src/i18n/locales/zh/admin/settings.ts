    ...
    settings: {
      ...
      features: {
        excelBpsImages: {
          ...
          nativeRetentionHint: '支持 PNG、JPEG、GIF 和 WebP；单张 20 MiB，每请求最多 {maxImages} 张、合计 32 MiB。本机不落盘，仅缓存附件 ID（每进程最多 512 条，30 分钟到期）。缓存按账号、凭据、API Key 和会话隔离；没有会话标识时不跨请求复用。图片上传至上游后的保存期限由上游决定。',
          ...
          limitRange: '范围：1–{max}',
          invalidLimits: '图片限制须为所示范围内的整数。每请求图片总大小不能小于单张上限；暂存容量和图片数须能容纳一次请求。',
          maxImageMiB: '单张图片上限 (MiB)',
          maxImages: '每请求图片数上限',
          maxTotalMiB: '每请求图片总大小 (MiB)',
          storageMiB: '进程暂存容量 (MiB)',
