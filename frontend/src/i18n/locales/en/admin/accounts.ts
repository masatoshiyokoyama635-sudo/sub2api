export default {
    accounts: {
      ...
      status: {
        ...
        turnStateHold: 'No candidate · {model} is paused on this account while the hunter collects a candidate; resumes on a match and pauses again after expiry if still empty',
        turnStateHoldShort: 'No candidate'
      },
      columns: {
        name: 'Name',
        id: 'Account ID',
      ...
      usageWindowsHint: "“5h / 7d” are rolling quota windows set by the upstream account provider, independent of sub2api settings and model mappings. Quota resets when the window rolls over. Turn-state rows below show candidates by model; their percentages and countdowns show remaining validity, not hit rate or model quality. Purple indicates a matching candidate shape and amber an unmatched shape. Shape cannot confirm the actual response model.",
      ollamaCloud: {
      ...
      openai: {
        ...
        turnStateAutoDesc:
          "Maintains candidates per account and model. Current shape rules accept 292 characters / 10 blocks for individual accounts and 332 characters / 12 blocks for Team accounts. An unmatched session state triggers a reuse attempt; an unmatched response after injection invalidates that candidate and tries the next one. If all candidates fail, the account is disabled with a recorded reason. Candidates expire one hour after minting by default, after which the system waits for new candidates. Manual overrides are ignored while enabled. HTTP paths only. Matching or unmatched shapes cannot confirm the actual response model or guarantee model quality.",
        ...
        turnStateHunter: "Turn-state hunter",
        turnStateHunterDesc:
          "Before a ticket expires, probe through the selected proxies using fresh sessions and pool states matching the candidate shape rules: currently 292 characters / 10 blocks or Team 332 characters / 12 blocks. Probes close after receiving headers; their main cost is input tokens, including the model base prompt. Available candidates are tried for all sessions on that model. Hourly and recent-traffic limits apply. Each probe opens a new proxy connection; rotating endpoints select a new exit. Fixed exits are probed once per round, with a seven-day cooldown after an unmatched shape. A matching shape does not guarantee the response model, and an unmatched shape does not confirm a model change.",
        ...
        turnStateHunterRotating: "Tick proxies that change exit with each new connection (webshare -rotate is detected automatically). Unticked proxies use fixed-exit scheduling: once per round, with a seven-day cooldown after an unmatched shape. This only affects hunter probes; normal requests keep the account’s configured proxy.",
        ...
        turnStateHunterHold: "Pause model scheduling when no candidate is available",
        turnStateHunterHoldDesc:
          "If a hunted model has no valid injectable candidate, pause that model on this account for one idle window (idle_minutes) and try another account, or return 503 if none is available. After expiry, the next request pauses it again if still empty; a new candidate resumes it immediately. Other models are unaffected. This controls candidate availability and does not confirm a model change.",
        turnStateRecovery: "Account exit state recovery probes",
        turnStateRecoveryDesc:
          "Probes through the account’s original proxy at randomized intervals. A matching streak under the current shape rules (292 characters / 10 blocks or Team 332 characters / 12 blocks) marks shape recovery; repeated failures trigger cooldown. Runs independently of the hunter, records markers and logs, and never changes settings. Probing stops after marking, and a subsequent unmatched state from real traffic restarts counting. The marker does not guarantee the response model or model quality.",
        turnStateRecoveryUsageKey: 'Usage API key ID (blank: use hunter setting)',
        turnStateRecoveryUsageKeyDesc: 'When blank, uses the hunter’s usage key even if the hunter is disabled. Probe usage is not recorded locally only when neither setting provides a key. The selected key follows normal billing and quota rules.',
        ...
        turnStatePool: {
          ...
          healthy: "matching candidate shape",
          suspect: "unmatched shape",
          ...
          recoveryDone: "state shape matched · {time}",
          hunterGateIdle: 'paused: no traffic',
          hunterGateHeld: "model paused: collecting candidates",
