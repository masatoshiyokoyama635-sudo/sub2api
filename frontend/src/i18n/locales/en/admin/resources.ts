export default {
    ...
    usage: {
      ...
      turnStateCopied: 'Turn-state copied',
      ...
      turnStateHint: "{chars} characters ({blocks} ciphertext blocks); under PKCS#7 the plaintext falls in {min}–{max} bytes. Current candidate shapes are 292 characters / 10 blocks and Team 332 characters / 12 blocks. This describes state shape only; matching or unmatched shapes cannot confirm the actual response model or guarantee model quality. Minted {minted}",
	  ...
	  turnStateFilter: 'Turn-state',
	  turnStateAll: 'All turn-state states',
	  turnStateMintedOnly: 'Upstream minted a new value',
	  turnStateHealthyOnly: "New length matches candidate lengths (292/332)",
	  turnStateSuspectOnly: "New length does not match candidate lengths (not 292/332)",
	  turnStateSentOnly: 'Request carried a turn-state',
	  turnStateInjectedOnly: 'Outbound value was overridden',
