...
## v0.1.183 自定义发布（2026-08-26，Actions 已发布，待 OVH 手动部署）
- [x] 从 v0.1.181 发布记录提交 `2943e164bceef98f1b1c6d70ae7ced1c8cc8d302` 创建 `backup/pre-v183-2943e164b` 与候选分支 `merge/v0.1.183-chat-image-tools`，合入官方 annotated tag object `c21fd3382a1c39fe491a96ac6780bac927327ae4`（peeled commit `e8cb019fabf8b55199436229044cbf9aa7a82564`）；官方 v0.1.183 完整包含 v0.1.182，比较范围为 39 个提交、66 个文件。
- [x] 解决 VERSION 与 WS v2 adapter 两处文本冲突；官方 tag 内 VERSION 仍为 `0.1.182`，已把 `backend/cmd/server/VERSION` 和嵌入断言统一为 `0.1.183`。WS 合并保留 Responses Lite normalize、图片权限门、reserved-tool alias、account identity 和 fast policy 的组合顺序，并新增首帧 Lite/第二轮图片意图回归测试。
- [x] 接入 Responses tool-call item ID 恢复、邮箱 alias 并发绑定、Kimi K3/composite 与 403 处理、Antigravity token/model routing、Channel Monitor composite SQL、Anthropic cache TTL 计费及 OpenAI OAuth 429/WS ingress 修复；保留 AI Chat、AI Images、Canvas、默认 CNY/动态币种、Prompt Audit、Strict Step-up、原子设置、安全 shutdown、partial/disconnect exactly-once 计费、Grok CAS 和 Custom Docker workflow。
- [x] 本地验证通过：Go 1.27.0 全包编译、全量 unit、`go vet -tags=unit ./...`、`golangci-lint v2.13.0`（`0 issues`）；前端 ESLint、typecheck、Vitest `249/249` files / `1798/1798` tests、production build；Apple container、Compose security/gateway env/runtime resources 和 Caddy cache/SSE 五项 shell 门禁。完整 integration 在本机因无 Docker daemon 不能启动 PostgreSQL，GitHub CI 的相同 unit/integration 门禁已全部通过。
- [x] 回滚制品位于 `.cache/update-v0.1.183/`：VERSION 原始 SHA-256 为 `14fa5f8b83ec64d482889a259fe6018c287813cc5fc02a99a09d200a6bf7030b`，修改版为 `bfea5cf6f3ead8cdf50848e4eed72ded56d58c9fb12d7e8b13afe7056defadb7`；`ROLLBACK.sh` 已在独立副本执行并恢复 `0.1.181`，源码与 `MODIFIED_FILE` 保持 `0.1.183`。
- [x] merge commit `dc2c7faf32e58944d73f8cd60d412f0e5647e019` 已推送到 fork `feature/chat-image-tools`。CI run `32925340186`、Security Scan run `32925340185`、Custom Docker Image run `32925340195` 全部成功。
- [x] GHCR 标签 `chat-image-tools`、`chat-image-tools-dc2c7fa`、`chat-image-tools-dc2c7faf32e58944d73f8cd60d412f0e5647e019` 均指向 OCI index `sha256:aec0f3df60b301ffb412863684f0e5a3536aeba919d6721606d65f776441e77a`；linux/amd64 child 为 `sha256:42a0f8af33c25732e51738730ffd21fb97388659a9e96a2b8e0b2aa0107a6a44`，linux/arm64 child 为 `sha256:ec5d82bee7490ad2f74550951c882f120d4cc06d18ff3f34258e55f7b6830eef`，两边 revision 均已核验为完整 merge SHA。
- [x] OVH 只读检查确认当前仍运行 v0.1.179 镜像/revision `80f7809f2860db8f94ef08ea36268b55815d4eeb`，应用、PostgreSQL、Redis 均 healthy，应用 restart count 为 0，`pull_policy: never`，数据盘剩余 363 GB。v0.1.181/v0.1.182 均未部署；更新前先生成 PostgreSQL dump，随后显式 pull v0.1.183 完整 SHA+digest，并只重建 `sub2api` 服务。

## v0.1.184 自定义发布（2026-08-31，Actions 已发布，待 OVH 手动部署）
- [x] 从 v0.1.183 自定义基线 `c1a3a123380762f34d8e5e198d3bf6b4a882a5ca` 创建 `backup/pre-v184-c1a3a1233` 与候选分支 `merge/v0.1.184-chat-image-tools`，合入官方 `v0.1.184` peeled commit `e98ef32eb29aecd30d1def615912ec4dc93173f3`；最终 merge commit 为 `83b345b9c8e68dedda7eceac9811c1dbc7d358ec`，`backend/cmd/server/VERSION` 与嵌入版本断言统一为 `0.1.184`。
- [x] 三处文本冲突均按语义并集解决：Responses-to-Chat fallback 同时保留 `ClientDisconnect` 与 `UpstreamResponseServiceTier`；WS v2 lifecycle fixture 同时保留禁图 API key/group 与上游 hooks；更新服务保留 custom lifecycle guard、标准 semver 比较并只规范化自定义 `-zz` 后缀，RC/beta 排序保持正确。
- [x] 保留 AI Chat、AI Images、Canvas、默认 CNY/动态币种、Prompt Audit、Strict Step-up、原子设置、安全 shutdown、partial/disconnect exactly-once 计费、图片权限门、Grok identity-bound billing CAS 和 Custom Docker workflow；接入上游 DeepSeek 峰谷计费、OpenAI/Anthropic/Bedrock failover、Codex catalog、图片冷却、TTFT semantic 模式及相关修复。
- [x] 本地门禁全部通过：Go 全包无 tag 编译、全量 unit/integration、`go vet -tags unit ./...`、golangci-lint 2.13.0（`0 issues`）；前端 ESLint、Vue typecheck、Vitest `254/254` files / `1857/1857` tests、production build；六项 workflow 部署 shell 门禁与 `git diff --check`。
- [x] 回滚制品位于 `.cache/update-v0.1.184/`：VERSION 原始 SHA-256 为 `bfea5cf6f3ead8cdf50848e4eed72ded56d58c9fb12d7e8b13afe7056defadb7`，修改版为 `98d7eacc163dcc68f2d215662a882445d3873ded2d187c9146f00c8bb4b09633`；`ROLLBACK.sh` 已在独立副本执行并恢复 `0.1.183` 与原始哈希，源码与 `MODIFIED_FILE` 保持 `0.1.184`。
- [x] merge commit 已推送到 fork `feature/chat-image-tools`。CI run `33393872939`、Security Scan run `33393872961`、Custom Docker Image run `33393872877` 均为 `completed/success`。
...
## v0.2.0 自定义发布（2026-09-02，Actions 已发布，待 OVH 手动部署）
- [x] 从 v0.1.185 发布记录提交 `d5011109c9d2d3e8ebb4f0c7dc973bba66f37339` 创建 `backup/pre-v020-d5011109c` 与候选分支 `merge/v0.2.0-chat-image-tools`，合入官方 annotated tag object `dd07c4d8d484878e617c945cc8bacc304a5a6560`（peeled commit `aa236488351eb71e120fc2b6fb32e36b0374c918`）；官方相对 v0.1.185 为 60 个提交、148 个文件，三方合并无文本冲突，最终 merge commit 为 `bed05d0635823e32d38ff23e7c753bd74964be38`。
- [x] 官方 tag 内 VERSION 仍停在 `0.1.185`，已把 `backend/cmd/server/VERSION` 与嵌入断言统一为 `0.2.0`。25 个双方同时修改的文件保留语义并集，AI Chat、AI Images、Canvas、默认 CNY/动态币种、Prompt Audit、Strict Step-up、原子设置、安全 shutdown、partial/disconnect exactly-once 计费、图片权限门、Grok identity CAS、DeepSeek `PricingAt`、数据库就绪重试和 Custom Docker workflow 均保留。
- [x] 合并审计修复了新增 1h cache-write 定价的计费与复制边界：只配置 `cache_write_1h_price` 时 5m 继承基础 cache-write 价，显式 5m 零价仍保持免费，Fast/priority 请求按最终 cache-write 档位比例同步缩放 5m/1h；Channel/interval 的 1h 价、Fast/Flex 和 multiplier 指针全部深拷贝，普通价、区间价、零价与 priority TTL 均有回归测试。
- [x] 修复三个协议策略绕过：上游在 response ID 出现前 EOF/1000 时，只要客户端已发 `response.create` 就判定 missing-terminal 失败；native 与 v2 WS 后续省略 model 时 reasoning mapping 使用 session model，v2 `session.update` 切换模型后同样生效；`/v1/messages` 回退 raw Chat Completions 时继续执行 Beta Fast、group force、global filter/block，并把最终 tier 传入 Free Fast 计费。
- [x] 新增迁移 `232_channel_cache_write_1h_pricing.sql`、`232_group_force_openai_fast.sql`、`232_group_reasoning_effort_over_limit.sql`、`233_group_free_openai_fast.sql`。迁移器按完整文件名/checksum 跟踪同号 232 文件；integration schema 测试断言四张 pricing 表的 `NUMERIC(20,12)` 列，以及 force/free/reasoning 默认值。生产数据库必须具备 `ALTER` 权限，启动迁移可能等待表锁。
- [x] 最终本地验证通过：Go 1.27.0 无 tag 全包编译、全量 unit、`go vet -tags unit ./...`、golangci-lint 2.13.0（`0 issues`）、高风险计费/WS/Fast race；前端 typecheck、ESLint、Vitest `256/256` files / `1878/1878` tests、production build；六项部署 shell gate、Linux amd64/arm64 嵌入构建和 repository Linux amd64/arm64、Windows amd64 交叉编译。因本机无 Docker，真实 PostgreSQL integration 由 CI run `33608930254` 执行并成功；Security Scan run `33608930210`、Custom Docker Image run `33608930292` 同为 `completed/success`。
- [x] 回滚制品位于 `.cache/update-v0.2.0/`：VERSION 原始 SHA-256 为 `7e555e7a3aea1eb8fec32b2074e0879b0ec6bfbf211bd19d8495c1549b171c87`，修改版为 `1f930dd1f133c1f97a94fe3acb8db34372cf4c01ffdb2b3ff4ca72f9494121e9`；`ROLLBACK.sh` 已在独立副本恢复 `0.1.185`，源码与 `MODIFIED_FILE` 保持 `0.2.0`。
- [x] GHCR 标签 `chat-image-tools`、`chat-image-tools-bed05d0`、`chat-image-tools-bed05d0635823e32d38ff23e7c753bd74964be38` 均指向 OCI index `sha256:19e58de459e5b4c1b8b9907cd2445d80907d0e98057075bc05e5bdfe45000844`；linux/amd64 child 为 `sha256:1d577a2f50fad34d6b5cba132e78b4c46e2325f2c842b98e0bf04a8d60c2f76a`，linux/arm64 child 为 `sha256:8b2bf7e2b841eb54b245438241718585ba2b01bb2c2b608a32c8e292117c5a13`，两边 revision 均核验为完整 merge SHA。
- [x] OVH 只读检查确认 `/data/sub2api/docker-compose.target.json` 当前固定 v0.1.185 完整 SHA 标签与 digest，`pull_policy: never`；应用、PostgreSQL、Redis 均 healthy，应用 restart count 为 0。本轮尚未部署；用户手动升级前先生成 PostgreSQL dump，再替换为 v0.2.0 完整 SHA+digest 并只重建 `sub2api`。
