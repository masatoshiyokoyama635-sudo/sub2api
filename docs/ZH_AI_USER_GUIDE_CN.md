...
- **无限画布**：新标签页打开 `https://canvas.zh-zh.top`；站点不会自动接收控制台 API Key，需要在画布内手动填写。
...
左侧菜单当前提供 **无限画布**，会在新标签页打开 `https://canvas.zh-zh.top`。跳转不会携带 API Key、登录 token、用户 ID 或其他参数，你需要在画布站点内手动填写 API Key。

旧版用户侧 **AI 生图** 页面仍作为兼容入口保留，需直接访问：
...
1. 登录控制台。
2. 直接打开 `https://ai.zh-zh.top/ai/images`（该兼容页不再显示在左侧菜单）。
3. 选择 **gpt-image 分组**。
