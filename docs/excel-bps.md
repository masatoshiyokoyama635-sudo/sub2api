...
2. 勾选 "遇到 BPS 403 错误时自动调整分组".
3. 明确选择一个 OpenAI 或复合分组, 或选择 "退出所有分组", 然后保存. 简易模式仅允许选择 OpenAI 分组.
4. 需要同时关闭协议时, 再勾选已有的自动关闭协议选项. 两项设置独立生效.

- 仅在实际 BPS Responses 上游返回 HTTP 403 时执行, 与现有自动关闭协议选项使用相同触发入口. 模型权限错误, 图片附件上传错误, 其他状态码和本地协议转换错误不触发分组调整.
- 移入指定分组会替换账号的全部现有分组. 已存在的目标分组成员关系保留优先级和模型限制. 退出所有分组会清空全部成员关系.
- 执行前重新检查账号凭据, 协议开关, 自动调整设置, 当前分组和目标分组. 配置或成员关系已变化时跳过; 目标不存在或平台不兼容时保留原分组.
- 分组修改与调度刷新事件在同一数据库事务中提交. 并发 403 只执行一次有效调整, 不重试当前失败请求, 不停用账号, 不影响其他账号.
- API 配置位于账号 extra: openai_excel_bps_auto_move_on_403 为布尔值, openai_excel_bps_403_target_group_id 为目标分组 ID. 数值 0 明确表示退出所有分组; 缺失或非法值不能代替 0.
...
- 工具回放缓存位于当前进程，最多 1024 条、单条 JSON 最多 1 MiB，总预算 16 MiB（包含 JSON、键、调用签名及估算的元数据开销，不代表进程 RSS）。命中后刷新空闲时间，连续 2 小时未使用的条目在下一次读写缓存时回收。重启、跨实例、换账号、淘汰或过期后，完整客户端工具历史可按既有规则重建；仅有工具结果、缺少原始调用时会报错。
...
- 普通 function 参数按声明的 JSON Schema 校验，包括 required、类型、枚举、组合与数值约束；保留大整数，不读取远程或本地外部 schema。单个 schema 上限 1 MiB；校验前限制嵌套 128 层、单个数字文本 4096 字符、指数绝对值 4096 及累计数值展开预算，超限明确拒绝。显式 FUNCTION_CMD 保留客户端校验元数据原值，仍检查声明目标、cmd 类型、封装大小与数值预算；普通 JSON function 封装继续执行完整 schema 校验。整批工具验证后再提交回放缓存和发送工具事件，重复 call_id、显式禁用并行却返回多个工具时拒绝整批。
...
- BPS 主请求（包括图片和 compact）返回 HTTP 429 时，客户端收到 `basispoints_rate_limited`，保留 HTTP 429；已开始 SSE 的请求收到失败事件。错误不会触发账号切换，也不会写入 Codex 用量快照、账号全局冷却或运行时封禁。真实 Codex 请求仍沿用原有限流保护。
- 工具纠正请求遇到 429 时立即结束纠正，沿用已有失败终态和用量汇总，不触发 Codex 冷却。成功响应的用量快照更新保持原有行为。BPS 配额仍需等待上游恢复，此改动不会增加可用额度。
...
- 先校验完整请求与所有内联图片，再使用选中账号的 OAuth、ChatGPT Account ID 和业务代理向固定的 BPS attachments 端点提交 multipart 文件，将图片替换为上游返回的附件 ID，最后发送 Responses。启用 Mihomo 时，先取得出口再上传，附件、生成与纠错共享该出口；本次请求已上传附件后不再切换出口重试。用户消息和 function/custom 工具截图均覆盖；HTTPS URL、相邻文本、工具参数及图片 detail 保持原值。
...
1. 打开管理员后台的 `系统设置 > 功能开关 > Excel / BPS 图片支持`.
2. 开启 `启用图片支持`, 选择临时 HTTPS 中转, 在 `公网 HTTPS 访问地址` 中填写当前服务的公网地址, 例如 `https://your-api.example.com`. 只填写 HTTPS origin, 不附加 `/v1`, 查询参数或账号密码.
...
- 启用任一图片传输方式后, 在 API Key 鉴权之后, 读取请求体和模型白名单校验之前执行共享准入. 作用于 OpenAI/Composite 分组的 Responses, Chat Completions 和 Messages HTTP 入口, 包括对应别名和 compact. 账号选择发生在读体之后, 因此纯文本及最终选择非 BPS 账号的请求也受该限制. 关闭图片支持后恢复原有入口限制.
- 管理员可设置请求体上限 (1–128 MiB)、估算资源预算 (512–2048 MiB) 和每进程最大在途请求数 (1–512), 默认分别为 64 MiB、1024 MiB 和 128 个. 配置预算必须至少为请求体上限的 8 倍; 实际准入预算取配置预算与在途上限 × 8 MiB 的较大值, 因此 512 槽至少对应 4096 MiB. 该预算用于资源记账, 不会预先分配等量内存. 已明确保存的容量配置保持原值, 新默认仅用于未保存的字段; 已配置的更小网关上限仍生效. 已知长度按请求体大小的 8 倍预留, 最小按 1 MiB 计算; 未知长度的未压缩请求随读取量增加预留, 压缩请求在解压期间按配置的请求体上限预留. 正文读取完成后, 未知长度和压缩请求按实际大小保留预算至请求结束. 大请求可用并发低于配置上限.
