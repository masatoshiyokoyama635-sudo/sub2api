...
- 单个请求体最多 64 MiB, 已配置的更小网关上限仍生效. 每进程最多 32 个在途请求, 同时使用 512 MiB 的估算资源预算: 已知长度按请求体大小的 8 倍预留, 最小按 1 MiB 计算; 未知长度按允许的最大请求体预留, 压缩请求按 64 MiB 解压上限预留. 大请求可用并发低于 32, 默认未知长度或压缩请求最多同时接纳 1 个.
- 预留持续到整个 HTTP 请求结束, 包括调度等待和上游流式响应阶段. 没有预算时在读体前返回 503 和 `Retry-After: 1`, 不建立应用层等待队列. 超大请求返回 413. 图片下载另有每进程 32 个并发上限.
- 512 MiB 是请求准入的估算预算, 不是进程 RSS 或容器的硬上限. JSON 解析, Go 运行时, 文件系统页缓存, 反向代理及其他业务仍消耗资源. 磁盘配额也不包含其他进程会话和待清理的崩溃遗留目录. 本功能不保证 200 人同时上传大图全部成功, 更不等同于整机不会 OOM.
