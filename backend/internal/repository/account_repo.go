...
// UpdateGrokBillingSnapshotIfIdentityUnchanged atomically merges the billing
// snapshot only when the complete Grok OAuth identity is still current.
func (r *accountRepository) UpdateGrokBillingSnapshotIfIdentityUnchanged(
	ctx context.Context,
	id int64,
	expected service.GrokBillingProbeIdentity,
	...
	if billing == nil {
		return false, service.ErrGrokBillingProbeIdentityChanged
	}
	credentials := expected.CredentialsJSON
	if strings.TrimSpace(credentials) == "" {
		return false, service.ErrGrokBillingProbeIdentityChanged
	}
	payload, err := json.Marshal(map[string]any{service.GrokBillingExtraKey: billing})
	if err != nil {
		return false, err
	}
	var proxyID any
	...
	`, string(payload), id, service.PlatformGrok, service.AccountTypeOAuth, credentials, proxyID)
	if err != nil {
		return false, err
	}
	affected, err := result.RowsAffected()
