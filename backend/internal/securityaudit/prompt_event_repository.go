...
func buildEventWhere(filter EventFilter, firstIndex int) (string, []any) {
	...
	if filter.Keyword != "" {
		add(` AND (e.request_id ILIKE $%d ESCAPE E'\\' OR e.prompt_hash ILIKE $%d ESCAPE E'\\' OR e.redacted_preview ILIKE $%d ESCAPE E'\\'
			OR e.username_snapshot ILIKE $%d ESCAPE E'\\' OR e.user_email_snapshot ILIKE $%d ESCAPE E'\\' OR e.api_key_name_snapshot ILIKE $%d ESCAPE E'\\')`, "%"+escapeLikePattern(TrimRunes(filter.Keyword, 128))+"%")
		// The clause has six placeholders but add only supplied one. Rebuild it with one shared placeholder.
		clauses[len(clauses)-1] = fmt.Sprintf(` AND (e.request_id ILIKE $%[1]d ESCAPE E'\\' OR e.prompt_hash ILIKE $%[1]d ESCAPE E'\\' OR e.redacted_preview ILIKE $%[1]d ESCAPE E'\\'
			OR e.username_snapshot ILIKE $%[1]d ESCAPE E'\\' OR e.user_email_snapshot ILIKE $%[1]d ESCAPE E'\\' OR e.api_key_name_snapshot ILIKE $%[1]d ESCAPE E'\\')`, firstIndex+len(args)-1)
	}
	if filter.StartAt != nil {
