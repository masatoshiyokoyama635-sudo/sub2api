	...
	for _, enabled := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			if enabled {
				...
				// Keep this fork's direct-image route. Its payload is not a Responses
				// body, so the new wire profile must not compress/reorder it.
				require.Equal(t, "/backend-api/codex/images/generations", up.lastReq.URL.Path)
				...
				require.Equal(t, wantInstall, up.lastReq.Header.Get("x-codex-installation-id"))
				// The retained direct-images endpoint does not send Responses beta.
				require.Empty(t, up.lastReq.Header.Get("OpenAI-Beta"))
			}
			require.Equal(t, resolveCodexOutboundIdentity("").version, up.lastReq.Header.Get("version"),
				"version 是 provider 头（model-provider-info/src/lib.rs:397），钉到规范身份")
