...
func TestExcelBPSToolCorrectionModelAccessDeniedDoesNotDisable(t *testing.T) {
	for _, stream := range []bool{false, true} {
		t.Run(fmt.Sprintf("stream=%t", stream), func(t *testing.T) {
			assertExcelBPSToolCorrection403Policy(t, stream, "basispoints_model_access_changed", true, 0)
		})
	}
}

func TestExcelBPSToolCorrectionOther403RespectsAutoDisable(t *testing.T) {
	for _, stream := range []bool{false, true} {
		for _, optIn := range []bool{false, true} {
			t.Run(fmt.Sprintf("stream=%t/enabled=%t", stream, optIn), func(t *testing.T) {
				wantCalls := 0
				if optIn {
					wantCalls = 1
				}
				assertExcelBPSToolCorrection403Policy(t, stream, "permission_denied", optIn, wantCalls)
			})
		}
	}
}

func assertExcelBPSToolCorrection403Policy(t *testing.T, stream bool, code string, optIn bool, wantCalls int) {
	t.Helper()
	gin.SetMode(gin.TestMode)
	first := &excelBPSRepairBody{Reader: strings.NewReader(excelBPSRepairWire(t, "forbidden_policy", "Run"))}
	raw, err := json.Marshal(map[string]any{"error": map[string]any{"code": code, "message": "PRIVATE_UPSTREAM"}})
	require.NoError(t, err)
	rejected := &excelBPSRepairBody{Reader: bytes.NewReader(raw)}
	upstream := &httpUpstreamRecorder{responses: []*http.Response{
		{StatusCode: http.StatusOK, Header: http.Header{}, Body: first},
		{StatusCode: http.StatusForbidden, Header: http.Header{}, Body: rejected},
	}}
	account := excelAccount()
	account.Extra["openai_excel_bps_auto_disable_on_403"] = optIn
	svc := openAIClientToolsTestService(upstream)
	calls := 0
	...
	body, err := json.Marshal(map[string]any{
		"model": "gpt-5.6-sol", "stream": stream, "input": "test correction rejection",
	...
	result, err := svc.Forward(context.Background(), c, account, body)
	require.Error(t, err)
	require.Len(t, upstream.requests, 2, "do not replay the forbidden correction")
	require.True(t, first.closed.Load())
	require.True(t, rejected.closed.Load())
	require.NotNil(t, result)
	require.Equal(t, 10, result.Usage.InputTokens)
