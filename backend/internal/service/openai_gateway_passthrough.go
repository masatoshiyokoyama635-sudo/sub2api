...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	imageIntent := resolveOpenAIPassthroughImageIntent(
		...
		IsExplicitImageGenerationIntent,
	)
	...
	for {
		...
		if reqStream {
			...
			if handleErr != nil {
				...
				if result == nil || !shouldReturnOpenAIPartialResult(result.usage, result.imageCount, 0, handleErr) {
					_ = resp.Body.Close()
	...
	} else if responseErr == nil && account.ParentAccountID != nil {
...
func (s *OpenAIGatewayService) buildUpstreamRequestOpenAIPassthrough(
	...
	req = s.prepareCodexTurnStateHTTP(c, account, body, req)
