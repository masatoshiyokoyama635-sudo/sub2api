...
func (b *Bridge) repairResponse(ctx context.Context, original object, repair RepairToolCall, observeUsage func(object)) (object, int, error) {
	reader, err := repair(ctx)
	if err != nil {
		return nil, -1, err
	}
	// Close must also interrupt a correction reader that has already been
	// returned by the callback and is not itself context-aware.
	...
	if observeUsage != nil {
		// Capture the raw second attempt before cumulative downstream merging,
		// including known usage returned with a failure or canceled read.
		observeUsage(corrected)
	}
