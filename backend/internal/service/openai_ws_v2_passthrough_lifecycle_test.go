...
func TestPassthroughLifecycle_SecondTurnImageIntentIsRejected(t *testing.T) {
	...
	defer cancelControl(context.Canceled)
	upstream := newStagedPassthroughConn()
	upstream.Send(`{"type":"response.completed","response":{"id":"resp_text_first","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`)
	server, serverErr := startPassthroughLifecycleServer(t, controlCtx, newPassthroughLifecycleService(passthroughLifecycleConfig(), upstream), passthroughLifecycleAccount())
	defer server.Close()
	clientConn := dialPassthroughLifecycleClient(t, server)
	defer func() { _ = clientConn.CloseNow() }()
	...
	completed, err := readPassthroughLifecycleFrame(t, clientConn, 3*time.Second)
	require.NoError(t, err)
	require.Equal(t, "resp_text_first", gjson.GetBytes(completed, "response.id").String())

	writeCtx, cancelWrite := context.WithTimeout(context.Background(), time.Second)
	err = clientConn.Write(writeCtx, coderws.MessageText, []byte(`{"type":"response.create","model":"gpt-5.1","previous_response_id":"resp_text_first","tools":[{"type":"image_generation"}],"tool_choice":"auto","input":"draw"}`))
	cancelWrite()
	require.NoError(t, err)
	...
	case unexpected := <-upstream.writes:
		t.Fatalf("forbidden second turn reached upstream: %s", unexpected)
	...
	case <-time.After(3 * time.Second):
		t.Fatal("passthrough image-intent gate did not terminate the connection")
	}
}
